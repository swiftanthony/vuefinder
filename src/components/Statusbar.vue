<template>
  <div class="p-1 text-xs border-t border-neutral-300 dark:border-gray-700/50 flex justify-between select-none">
    <div class="flex leading-5 items-center">
      <template v-if="options.statusbar.storage">
    ...
    <div class="flex leading-5 items-center">
      ...
      <select v-model="locale" @change="changeLocale($event.target.value)" v-if="options.statusbar.locale !== false"
      ...
      <span class="mr-1" :aria-label="t('About')" data-microtip-position="top-left" role="tooltip"
            v-if="options.statusbar.about"
            @click="emitter.emit('vf-modal-show', {type:'message', title:'Vuefinder ' + version, message: t('Vuefinder is a file manager component for vue 3.')})">
